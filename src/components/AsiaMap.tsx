...
// Projection bounds (lng 68→148, lat 48→-12) onto the viewBox.
const W = 720
const H = 540
const x = (lng: number) => ((lng - 68) / (148 - 68)) * W
const y = (lat: number) => ((48 - lat) / (48 + 12)) * H
...
export function AsiaMap({ live, coming }: { live: string; coming: string }) {
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label="StayEasy expansion across Asia">

...
export default function PartnerResetPage() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    if (pw.length < 6) return setMsg({ kind: 'error', text: t.reset.errPwShort })
    if (pw !== pw2) return setMsg({ kind: 'error', text: t.reset.errMismatch })
    const ok = partnerAccounts.resetPassword(email.trim(), pw)
    if (!ok) return setMsg({ kind: 'error', text: t.reset.errNoAccount })
    setMsg({ kind: 'ok', text: t.reset.okUpdated })
    ...
    <div className="grid min-h-screen place-items-center bg-sand-50 px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={submit} className="rounded-3xl bg-white p-7 shadow-card ring-1 ring-black/5">
          <h1 className="text-xl font-extrabold text-ink-900">{t.reset.title}</h1>
          <p className="mt-1 text-sm text-ink-700/70">{t.reset.subtitle}</p>
          ...
          <div className="mt-5 space-y-4">
            ...
            <label className="block">
              <span className={labelCls}>{t.reset.newPw}</span>
              <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} required placeholder={t.login.pwMinPh} className={`mt-1 ${inputCls}`} />
            </label>
            <label className="block">
              <span className={labelCls}>{t.reset.confirmPw}</span>
              <input type="password" value={pw2} onChange={(e) => setPw2(e.target.value)} required className={`mt-1 ${inputCls}`} />
            </label>
          </div>

          <button type="submit" className="mt-6 w-full rounded-xl bg-brand-600 px-4 py-3 text-sm font-semibold text-white hover:bg-brand-700">{t.reset.updatePw}</button>
        </form>
        <div className="mt-4 text-center text-sm">
          <Link to="/partner/login" className="text-brand-700 hover:underline">{t.backToSignIn}</Link>
        </div>
        <p className="mt-3 text-center text-xs text-ink-600/55">{t.reset.demoNote}</p>
      </div>
    </div>
  )
}
